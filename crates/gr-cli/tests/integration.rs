...
use gr_core::address::{Address, AddressRange, AddressSet, SpaceId, Endian};
use gr_core::pcode::{OpCode, PcodeOp, SeqNum, VarnodeData};

#[test]
fn address_model_complete() {
    let mut mgr = gr_core::address::SpaceManager::new();
...
fn datatype_manager_builtins() {
    let mgr = gr_core::datatype::DataTypeManager::new();
...
fn segmented_address() {
    let seg = gr_core::address::SegmentedAddress::new(0x0800, 0x0100);
...
fn address_map_roundtrip() {
    let mut map = gr_core::address::AddressMap::new();
...
fn breakpoint_manager() {
    let mut mgr = gr_emulator::BreakpointManager::new();
...
fn sleigh_decision_tree() {
    use gr_sleigh::decision::{DecisionNode, PatternMatch};
...
#[test]
fn type_inference() {
    let t = gr_decompile::InferredType::Integer { size: 4, signed: true };
    assert_eq!(t.to_c_type(), "int32_t");

    let p = gr_decompile::InferredType::Pointer { pointee_size: Some(1) };
    assert_eq!(p.to_c_type(), "char*");
}

#[test]
fn project_summary_roundtrip() {
    use gr_program::project::*;
...
fn flirt_pattern_matching() {
    use gr_loader::flirt::{FlirtDatabase, FlirtPattern};
