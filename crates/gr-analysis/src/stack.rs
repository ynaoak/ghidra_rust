    ...
    fn analyze(&self, program: &mut Program) -> Result<AnalysisResult, AnalysisError> {
        ...
        for entry in func_entries {
            ...
            if let Some(func) = program.listing.get_function_mut(entry) {
                func.stack_frame.local_size = stack_alloc as u64;
            }
        }
...
fn detect_stack_access_from_ops(
    ...
    for op in ops {
        if op.opcode == OpCode::IntAdd
            && let Some(out) = &op.output
                && out.space == addr_vn.space && out.offset == addr_vn.offset {
                    ...
                    if let (Some(b), Some(d)) = (base, disp)
                            ...
                            && d.space == SpaceId::CONST
                        {
                            ...
                            if let Some(func) = program.listing.get_function_mut(func_entry) {
                                func.stack_frame.add_variable(offset, 8);
                                *total_vars += 1;
                            }
                        }
                }
